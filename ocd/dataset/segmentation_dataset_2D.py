...
class CancerSegmentationDataset(TorchDataset):
  ...
  def _get_metadata(self, idx: int) -> dict:
    ...
    ct_img = nib.loadsave.load(ct_path)
    seg_img = nib.loadsave.load(seg_path)

    return {
      "ct_path": ct_path,
      "seg_path": seg_path,
      "patient_id": os.path.basename(ct_path).split(".")[0],
      "ct_affine": ct_img.affine,  # pyright: ignore
      "seg_affine": seg_img.affine,  # pyright: ignore
      "ct_shape": ct_img.shape,  # pyright: ignore
      "seg_shape": seg_img.shape,  # pyright: ignore
    }

  def _get_slice(self, volume: np.ndarray, slice_idx: int | None = None) -> np.ndarray:
  ...
  def _build_slice_map(self):
    ...
    for vol_idx, (ct_path, _) in enumerate(self.paired_data):
      ...
      n_slices = img.shape[self.slice_axis]  # pyright: ignore
