...
class SampleUtils:
    ...
    """
    @classmethod
    def load_from_path(cls, file_path: str):
        """
    ...
    @classmethod
    def display_slice(cls, data, slice_idx=None, axis=2, figsize=(20, 16),
    ...
    @classmethod
    def normalize_ct(cls, ct_data: np.ndarray,
                     window_center: int = 40,
                     window_width: int = 400,
                     output_range: Tuple[float, float] = (-1, 1)) -> np.ndarray:
        """
...
class Dataset:
    ...
    def __init__(self, base_dir: str = 'DatasetChallenge'):
        ...
        # === MSKCC ===
        MSKCC_pairs = self._create_paired_dataset(
            datasets=self.data,
            dataset_name='MSKCC'
        )
        ...
        # === MSKCC ===
        TCGA_pairs = self._create_paired_dataset(
            datasets=self.data,
            dataset_name='TCGA'
        )
    ...
    def _create_paired_dataset(cls, datasets: Dict[str, List[str]], dataset_name: str = 'MSKCC') -> List[Tuple[str, str]]:
        """
    ...
    def _split_dataset(cls, paired_data: List[Tuple[str, str]],
                      train_ratio: float = 0.7,
                      val_ratio: float = 0.15,
                      test_ratio: float = 0.15,
                      random_state: int = 42,
                      dataset_name: str = "") -> Tuple[List, List, List]:
        """
        ...
        print(f"Dataset split {dataset_name}: Train={len(train_pairs)}, Val={len(val_pairs)}, Test={len(test_pairs)}")
