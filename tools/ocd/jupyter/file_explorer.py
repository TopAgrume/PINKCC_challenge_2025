...
class JupyterExplorer:
  @classmethod
  def file_selector(cls, dataset_dict):
    ...
    def on_button_click(b):
      with output:
        ...
        if file_dropdown.value:
          ...
          JupyterVisualizer.interactive_slice_viewer(
            data, title_prefix=f"{os.path.basename(file_dropdown.value)}: "
          )
  ...
  @classmethod
  def paired_file_selector(cls, ct_dict, seg_dict):
    ...
    def on_button_click(b):
      with output:
        ...
        if ct_file_dropdown.value and seg_file_dropdown.value:
          ...
          # Load the files
          ct_img = nib.load(ct_file_dropdown.value)  # pyright: ignore
          seg_img = nib.load(seg_file_dropdown.value)  # pyright: ignore

          ct_data = ct_img.get_fdata()  # pyright: ignore
          seg_data = seg_img.get_fdata()  # pyright: ignore
          ...
          if ct_data.shape == seg_data.shape:
            ...
            print(
              f"Warning: CT shape {ct_data.shape} doesn't"
              f" match segmentation shape {seg_data.shape}"
            )
